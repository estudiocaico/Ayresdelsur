...
function Tab({ to, icon, label }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        `flex flex-col items-center gap-0.5 px-5 py-2 text-[0.6rem] font-bold uppercase tracking-wider transition-colors ${
      ...
      <div className="min-h-dvh bg-cream flex flex-col">
        ...
        <header className="bg-negro text-white px-4 py-3 flex items-center justify-between sticky top-0 z-50 shadow-sm">
          <div className="flex items-center gap-2.5">
            <img src="/logo-circular.png" alt="Ayres del Sur" className="h-7 w-auto" />
            <span className="font-display font-bold text-[0.95rem] tracking-tight">Ayres del Sur</span>
          </div>
        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-cream-dark flex justify-around items-center h-[60px] z-50 shadow-[0_-2px_12px_rgba(0,0,0,0.06)]">
          <Tab to="/vendedor"           icon="📋" label="Mis visitas" />
          <Tab to="/vendedor/pedido"    icon="🛒" label="Nuevo pedido" />
          <Tab to="/vendedor/perfil"    icon="👤" label="Mi perfil" />
        </nav>
      </div>
